...
export default function ProjectGalleryAlbumSection({ project }: Props) {
  const images = useMemo(() => Array.from(new Set(project.gallery.images.filter(Boolean))), [project.gallery.images]);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowLeft") setActiveIndex((value) => value === null ? value : (value - 1 + images.length) % images.length);
      if (event.key === "ArrowRight") setActiveIndex((value) => value === null ? value : (value + 1) % images.length);
    };
    ...
    <section className="rounded-[24px] border border-line/80 bg-white p-4 shadow-soft sm:p-6 lg:p-8">
      <div className="mb-5 text-left">
        {project.gallery.label ? (
          <p className="text-[11px] font-bold tracking-[0.16em] text-gold normal-case">{project.gallery.label}</p>
        ) : null}
        <h2 className="heading-font mt-2 text-[24px] font-semibold leading-tight text-ink normal-case sm:text-[30px]">
          {project.gallery.title || "Album ảnh dự án"}
        </h2>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-muted sm:text-[15px]">
          {project.gallery.description || "Khám phá không gian sống, tiện ích, cảnh quan và những góc nhìn nổi bật của dự án."}
        </p>
      </div>
        ...
        {visibleImages.map((image, index) => {
          ...
          return (
            ...
            >
              <Image
                src={image}
                alt={`${project.gallery.title || project.name} ${index + 1}`}
                fill
      ...
      {activeIndex !== null ? (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/88 p-4" onClick={() => setActiveIndex(null)}>
          ...
          <img
            ...
            alt={`${project.gallery.title || project.name} ${activeIndex + 1}`}
