...
- Round 4 local full-QA SHA: recorded in `artifacts/qa/seo-schema-hardening-rc3/test-logs/commit-sha.txt` after the corrective code commit.
...
The authoritative Round 4 results are the logs in `artifacts/qa/seo-schema-hardening-rc3/test-logs/` generated by `scripts/run-seo-rc3-qa.ps1` at the SHA recorded in `commit-sha.txt`.

- `npm ci`: pass.
- `npm run lint`: pass with 0 errors; 406 existing warnings remain.
- `npx tsc --noEmit`: pass.
- `npm run test:seo:schema`: 8 tests pass, 0 fail.
- `npm run build`: pass on Next.js 16.2.7.
- Composer install and isolated SQLite testing migration: pass.
- Full backend test suite: pass; exact counts are in `backend-tests.log`.
- Asset validation: pass.
- SEO integration smoke: pass; fixture skips are recorded explicitly in `seo-smoke.log`.
- Docker is started only for backend/integration QA and is stopped in the runner's `finally` block.
